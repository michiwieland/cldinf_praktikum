...
\begin{description}
	...
	\item[VTEP: VXLAN Tunnel End Point]  
	\item[VEM: Virtual Ethernet Module] 
\end{description}
...
\subsection{Ist VXLAN kompatibel mit traditionellen Protokollen} % If yes, what are the differences and which limitations does VXLAN solve?

\subsection{Was sind die Vor- und Nachteile von VXLAN}

\subsection{Gibt es andere moderne Technologien, welche TRILL ähnlich sind?}%  if yes, what are the differences between them


VXLAN benötigt keine spezielle Hardware mehr. description
Die Verbindung zum restlichen Netzwerk fehlt bei Host Overlays -> Hybrid

ethernet in udp encapsulation

24Bit Identifier, vergleichbar mit VLAN (VLAN zwischen Hosts = VYLAN)
